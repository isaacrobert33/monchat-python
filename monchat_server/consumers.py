...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def single_chat(self, event):
        await self.send(text_data=json.dumps(event))
...
class GeneralReadReceiptConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        self.room_group_name = "general_read_receipt"
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        await self.channel_layer.group_send(
            self.room_group_name, {"type": "gen_chat_status_change", **data}
        )

    async def gen_chat_status_change(self, event):
        await self.send(text_data=json.dumps(event))
